...
> If you don't have Azure credentials, the server will still work — it will fall back to PaddleOCR for all requests.
...
This is a temporary test UI — upload `.png`, `.jpg`, `.jpeg`, or `.pdf` files and see extracted text.
...
|--------|-----------|------------------------------------|
| GET    | `/`       | Health check                       |
| POST   | `/upload` | Upload file for OCR text extraction|
...
```json
{
  "filename": "notes.pdf",
  "engine_used": "azure",
  "text_preview": "extracted text here..."
}
```
...
│       ├── api/
│       │   └── upload.py         # /upload endpoint with size/page validation
│       ├── core/
...
│           ├── preprocess.py     # Image preprocessing (grayscale, denoise)
...
- **OCR flow**: Upload → Azure OCR (primary) → PaddleOCR (fallback) → Return text
